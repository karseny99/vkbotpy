	...
	try:
		...
		if messages['count']>= 1:
			...
			if body.lower() == 'привет' or body.lower() == 'назад':
				vk.method('messages.send', {'peer_id': id, 'message': 'Дарова, епта, че надо', 'keyboard': keyboard,  'random_id': random.randint(1,231321321)})
			elif body.lower() == 'уроки':
				vk.method('messages.send', {'peer_id': id, 'message': 'ну выбирай, хуль', 'keyboard': keyboard1,  'random_id': random.randint(1,231321321)})
				...
				vk.method('messages.send', {'peer_id': id, 'message': 'У тебя все хорошо? Может к врачу сгоняешь?', 'random_id': random.randint(1,2312312321)})
			elif body.lower() == 'нет' or body.lower() == 'да' or 'схожу' in body.lower() or 'пойду' in body.lower():
				vk.method('messages.send', {'peer_id': id, 'message': 'Ну вот и пиздуй','random_id': random.randint(1,2312312321)})
			...
			7:🤡Обществознание🤡\n каб204
				...
				vk.method('messages.send', {'peer_id': id, 'sticker_id': 163, 'random_id': random.randint(1, 2147483647)})
				vk.method('messages.send', {'peer_id': id, 'message': '''
				1. ♿Физра♿
				2. ♂ Окно ♂
				3. 📵ОБЖ📵\n каб 404
				...
				vk.method('messages.send', {'peer_id': id, 'message': 'Ну щас тут значт ♂'+str(status)+'♂, а также '+str(round(temp))+'°C', 'random_id': random.randint(1, 2147483647)})
			elif body.lower() == 'создатель':
				vk.method('messages.send', {'peer_id': id, 'message': '@id222338543 (♂хто я♂)', 'random_id': random.randint(1, 2147483647)})
				...
				vk.method('messages.send', {'peer_id': id, 'message': 'соси', 'random_id': random.randint(1, 2147483647)})
